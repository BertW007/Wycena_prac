...
'''Read csv file and return a column'''
def read_col(CSV_FILE,CSV_COL):
    try:
        with open(CSV_FILE, 'rb') as f:
            ...
            reader = csv.reader(f)
            for row in reader:
                records.append(row[0].decode("utf-8-sig").encode("utf-8"))
            return records
    except IOError:
...
countrycodes = read_col(CSV_FILE = "spis.csv", CSV_COL = 0)
countrynames = read_col(CSV_FILE = "spis.txt", CSV_COL = 1)
...
def showPopulation(*args):
    ...
    if len(idxs)==1:
        ...
        name = countrynames[idx].decode("utf-8")
